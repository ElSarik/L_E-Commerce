...
<div class="sub">
	...
	<div>
		<button type="button" class="sub_categories" onclick="window.location.href = 'orders.php?filter=Received';">
			Received
		</button>
	</div>
	<div>
		<button type="button" class="sub_categories" onclick="window.location.href = 'orders.php?filter=Sent';">
			Sent
		</button>
	</div>
	<div>
		<button type="button" class="sub_categories" onclick="window.location.href = 'orders.php?filter=Completed';">
			Completed
		</button>
	</div>
</div>
...
<?php
	...
	if(isset($_GET["filter"]))
	{
		...
		$query1 = "SELECT * FROM orders WHERE status !='Open' AND status ='$filter'";
		...
		foreach($result1 as $order)
		{
			$output .= 
				"
					...
					<form action='orders.php' method='post' id='".$order['id']."'>
					<div class='OrdersDetails'>".$order['id']." |	<select  name='status_change' form='".$order['id']."' id='status_change'> <option value='' selected disabled hidden>".$order['status']."</option> <option value='Received'>Received</option> <option value='Sent'>Sent</option> <option value='Completed'>Completed</option> </select> <input name='order_id' id='order_id' type='hidden' value='".$order['id']."' /> <input name='changestatus_btn' type='submit' value='Update' /> | ".$order['address']." | ".$order['name']." ".$order['surname']." | ".$order['cost']."
					</div>
					</form>
				";
