...
<div class="sub"> 
	...
	<div>
			<button type="button" class="sub_categories" onclick="window.location.href = 'headphones.php';">
				<?php   
					$result = mysqli_query($db,"SELECT DISTINCT category FROM sound WHERE category = 'Headphones'");
					while($row = mysqli_fetch_array($result))
					{
	...
	<div class="FiltersLeft">
		<div>
			<ul class="FilterOrder">
				...
				<?php
					$result = mysqli_query($db,"SELECT DISTINCT(manufacturer) FROM sound WHERE category = 'headphones' AND manufacturer !=''");
		...
		<div>
			<ul class="FilterOrder">
				...
				<?php
					$result = mysqli_query($db,"SELECT DISTINCT(power) FROM sound WHERE category = 'headphones' AND power !=''");
                    foreach($result as $row)
                    {
                    ?>
		...
		<div>
			<ul class="FilterOrder">
				...
				<?php
					$result = mysqli_query($db,"SELECT DISTINCT(type) FROM sound WHERE category = 'headphones' AND type !=''");
                    foreach($result as $row)
                    {
                    ?>
...
$(document).ready(function(){
    ...
    function ProductDisplay()
    {
        ...
        $.ajax({
            url:"filters.php",
            method:"POST",
            data:{headphones:headphones, manufacturer:manufacturer, power:power, type:type, under50:under50, between50100:between50100, over100:over100},
