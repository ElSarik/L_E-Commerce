...
<div class="sub"> 
	...
	<div> 
		...
		<div>
			<button type="button" class="sub_categories" onclick="window.location.href = 'apple_mac.php';">		
				<?php   
					$result = mysqli_query($db,"SELECT DISTINCT category FROM computers WHERE category = 'AM'");
					while($row = mysqli_fetch_array($result))
					{
						if($row['category'] == 'AM')
						{
		...
		<div> 
			<button type="button" class="sub_categories" onclick="window.location.href = 'servers.php';">
				<?php   
					$result = mysqli_query($db,"SELECT DISTINCT category FROM computers WHERE category = 'Servers'");
					while($row = mysqli_fetch_array($result))
					{
						echo $row['category']."<br>";
					}
	...
	<div class="FiltersLeft">
		<div>
			<ul class="FilterOrder">
				...
				<?php
					$result = mysqli_query($db,"SELECT DISTINCT(manufacturer) FROM computers WHERE category = 'servers' AND manufacturer !=''");
		...
		<div>
			<ul class="FilterOrder">
				...
				<?php
					$result = mysqli_query($db,"SELECT DISTINCT(cpu) FROM computers WHERE category = 'servers' AND cpu !=''");
                    foreach($result as $row)
                    {
                    ?>
		...
		<div>
			<ul class="FilterOrder">
				...
				<?php
					$result = mysqli_query($db,"SELECT DISTINCT(ram) FROM computers WHERE category = 'servers' AND ram !=''");
                    foreach($result as $row)
                    {
                    ?>
		...
		<div>
			<ul class="FilterOrder">
				...
				<?php
					$result = mysqli_query($db,"SELECT DISTINCT(storage) FROM computers WHERE category = 'servers' AND storage !=''");
                    foreach($result as $row)
                    {
                    ?>
		...
		<div>
			<ul class="FilterOrder">
				...
				<?php
					$result = mysqli_query($db,"SELECT DISTINCT(gpu) FROM computers WHERE category = 'servers' AND gpu !=''");
                    foreach($result as $row)
                    {
                    ?>
	...
	<div class="FiltersRight">
		<div>
			<ul class="FilterOrder">
				...
				<?php
					$result = mysqli_query($db,"SELECT DISTINCT(screen) FROM computers WHERE category = 'servers' AND screen !=''");
                    foreach($result as $row)
                    {
                    ?>
		...
		<div>
			<ul class="FilterOrder">
				...
				<?php
					$result = mysqli_query($db,"SELECT DISTINCT(os) FROM computers WHERE category = 'servers' AND os !=''");
                    foreach($result as $row)
                    {
                    ?>
...
$(document).ready(function(){
    ...
    function ProductDisplay()
    {
        ...
        $.ajax({
            url:"filters.php",
            method:"POST",
            data:{servers:servers, manufacturer:manufacturer, cpu:cpu, ram:ram, storage:storage, gpu:gpu, screen:screen, os:os, under500:under500, between5001000:between5001000, over1000:over1000},
