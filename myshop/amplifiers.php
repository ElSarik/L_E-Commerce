	...
	<div class="FiltersLeft">
		<div>
			<ul class="FilterOrder">
				...
				<?php
					$result = mysqli_query($db,"SELECT DISTINCT(manufacturer) FROM sound WHERE category = 'amplifier' AND manufacturer !=''");
		...
		<div>
			<ul class="FilterOrder">
				...
				<?php
					$result = mysqli_query($db,"SELECT DISTINCT(power) FROM sound WHERE category = 'amplifier' AND power !=''");
                    foreach($result as $row)
                    {
                    ?>
		...
		<div>
			<ul class="FilterOrder">
				...
				<?php
					$result = mysqli_query($db,"SELECT DISTINCT(type) FROM sound WHERE category = 'amplifier' AND type !=''");
                    foreach($result as $row)
                    {
                    ?>
	...
	<div class="FiltersRight">
		<div>
			<ul class="FilterOrder">
				...
				<label> <li class="Filter"> <input type="checkbox" onclick="change_color(this);" class="common_selector under50" value="under50"> Under 500€</li></label>
				<label> <li class="Filter"> <input type="checkbox" onclick="change_color(this);" class="common_selector between50100" value="between50100"> 500€ - 1000€</li></label>
				<label> <li class="Filter"> <input type="checkbox" onclick="change_color(this);" class="common_selector over100" value="over100"> Over 1000€</li></label>
			</ul>
		</div>
	</div>
</div>
...
$(document).ready(function(){
    ...
    function ProductDisplay()
    {
        ...
        $.ajax({
            url:"filters.php",
            method:"POST",
            data:{amplifier:amplifier, manufacturer:manufacturer, power:power, type:type, under50:under50, between50100:between50100, over100:over100},
